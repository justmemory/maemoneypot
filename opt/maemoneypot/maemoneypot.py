...
database = '/home/user/.maemoneypot/.maemoneypotdb' #This is fixed to exclude 'dummy user' actions as the database content is definite
...
try:
    ...
    db = sql.connect(database)
    cur = db.cursor()
    cur.execute('CREATE TABLE Expenses(Id integer primary key, Date date, Expense int, Category varchar, Tag varchar);')
    cur.execute('CREATE TABLE Incomes(Id integer primary key, Date date, Income int, Category varchar);')
    cur.execute('CREATE TABLE Savings(Id integer primary key, Date date, Saving int, Category varchar);')
    iface.SystemNoteInfoprint('Database created')

from db_query import DatabaseQueries
from matplotlib_query import MatplotlibQueries
from clickable_pixbuf import CellRendererClickablePixbuf
...
class MainWindow(hildon.StackableWindow):
    ...
    def __init__(self):
        ...
        ButtonPreviousMonth = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ...
        ButtonNextMonth = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ButtonNextMonth.set_title('Next month')
        ButtonNextMonth.connect('clicked', self.goToNextMonth)
        Menu.append(ButtonNextMonth)
        ButtonSpecificMonth = hildon.PickerButton(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ...
        ButtonCalendarView = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        if defaultView == 'Main':
        ...
        ButtonSettings = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ButtonSettings.set_title('Settings')
        ButtonSettings.connect('clicked', self.settings)
        Menu.append(ButtonSettings)
        ButtonSearch = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
    ...
    def mainView(self, year, month, day):
        ...
        cur.execute('select sum(Expense) from Expenses where strftime("%Y", Date)="'+year+'" and strftime("%m", Date)="'+month+'";')
        list1.append(cur.fetchone()[0])
        cur.execute('select sum(Income) from Incomes where strftime("%Y", Date)="'+year+'" and strftime("%m", Date)="'+month+'";')
        ...
        if list2[0] is not None:
            label.set_text("<span size='21000'>"+str(list2[0]-list1[0])+"</span>")
    ...
    def calendarView(self, year, month):
        ...
        Month = month
        self.set_title(MonthNameList[month]+' | '+year)
        if calendar.monthrange(int(year), int(month))[0] == 5 or calendar.monthrange(int(year), int(month))[0] == 6:
            y = 7
        else:
            y = 6
        ...
        for i in range(1, calendar.monthrange(int(year), int(month))[1]+1):
            if i < 10:
                cur.execute('select sum(Expense) from Expenses where strftime("%Y", Date) = "'+year+'" and strftime("%m", Date) = "'+month+'" and strftime("%d", Date) = "0'+str(i)+'";')
            else:
                cur.execute('select sum(Expense) from Expenses where strftime("%Y", Date) = "'+year+'" and strftime("%m", Date) = "'+month+'" and strftime("%d", Date) = "'+str(i)+'";')
            expense = cur.fetchone()
            if i < 10:
                cur.execute('select sum(Income) from Incomes where strftime("%Y", Date) = "'+year+'" and strftime("%m", Date) = "'+month+'" and strftime("%d", Date) = "0'+str(i)+'";')
            else:
                cur.execute('select sum(Income) from Incomes where strftime("%Y", Date) = "'+year+'" and strftime("%m", Date) = "'+month+'" and strftime("%d", Date) = "'+str(i)+'";')
            income = cur.fetchone()
            if i < 10:
                cur.execute('select sum(Saving) from Savings where strftime("%Y", Date) = "'+Year+'" and strftime("%m", Date) = "'+Month+'" and strftime("%d", Date) = "0'+str(i)+'";')
            else:
                cur.execute('select sum(Saving) from Savings where strftime("%Y", Date) = "'+year+'" and strftime("%m", Date) = "'+month+'" and strftime("%d", Date) = "'+str(i)+'";')
            saving = cur.fetchone()

            vbox = gtk.VBox(False, 0)
            button = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
            ...
            if i == int(time.strftime('%d')) and year+month == time.strftime('%Y%m'):
                datelabel.modify_fg(gtk.STATE_NORMAL, gtk.gdk.color_parse('#f47500'))
            ...
            if expense[0] is not None:
                ...
                expenselabel.modify_fg(gtk.STATE_NORMAL, gtk.gdk.color_parse('#f40005'))
            ...
            if income[0] is not None:
                ...
                incomelabel.modify_fg(gtk.STATE_NORMAL, gtk.gdk.color_parse('#00f475'))
            ...
            if saving[0] is not None:
                ...
                savinglabel.modify_fg(gtk.STATE_NORMAL, gtk.gdk.color_parse('#007ff4'))
    ...
    def goToSpecificMonth(self, data = None):
        years = ['2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023', '2024', '2025', '2026', '2027', '2028', '2029', '2030', '2031', '2032', '2033', '2034', '2035', '2036', '2037', '2038', '2039', '2040', '2041', '2042', '2043', '2044', '2045']
    ...
    def settings(self, data = None):
        ...
        def saveSettings(self, data = None):
            if len(settings[0]) == 1 and len(settings[1]) == 1 and len(settings[2]) == 1:
                windowSettings.connect('destroy', gtk.main_quit)
            settings[0] = currencyNew+'\n'
            settings[1] = defaultViewNew+'\n'
            settings[2] = savingCategory+'\n'
            with open('/home/user/.maemoneypot/settings.ini', 'w') as settingsfile:
        ...
        buttonDefaultView = hildon.PickerButton(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ...
        buttonSave = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        ...
        table.attach(buttonSave, 0, 1, 4, 5)
    ...
    def search(self, widget):
        ...
        dialog = gtk.Dialog('Search...', self, gtk.DIALOG_NO_SEPARATOR | gtk.DIALOG_DESTROY_WITH_PARENT)
        ...
        if response == gtk.RESPONSE_OK:
            for i in xrange(len(allExpense)):
                for j in xrange(len(allExpense[i])):
                    if str(searchphrase).lower() in str(allExpense[i][j]).lower():
                        tempAllExpense.append(allExpense[i])
            for i in xrange(len(allIncome)):
                for j in xrange(len(allIncome[i])):
                    if str(searchphrase).lower() in str(allIncome[i][j]).lower():
                        tempAllIncome.append(allIncome[i])
            for i in xrange(len(allSaving)):
                for j in xrange(len(allSaving[i])):
                    if str(searchphrase).lower() in str(allSaving[i][j]).lower():
                        tempAllSaving.append(allSaving[i])

            self.viewSearchResultsBrief(tempAllExpense, tempAllIncome, tempAllSaving, searchphrase)
        dialog.destroy()

    def viewSearchResultsBrief(self, allExpense, allIncome, allSaving, searchphrase):
        def elements():
            ...
            if len(allExpense) == 0 and len(allIncome) == 0 and len(allSaving) == 0:
                ...
                resultsExpense.set_text(str(len(allExpense))+' results found in Expenses database.')
                resultsIncome = gtk.Label()
                resultsIncome.set_text(str(len(allIncome))+' results found in Incomes database.')
                resultsSaving = gtk.Label()
                resultsSaving.set_text(str(len(allSaving))+' results found in Savings database.')
        ...
        pannableArea = hildon.PannableArea()
        pannableArea.add_with_viewport(elements())
        dialog = gtk.Dialog('Search results', self, gtk.DIALOG_NO_SEPARATOR | gtk.DIALOG_DESTROY_WITH_PARENT)
        dialog.set_size_request(0, 200)
        dialog.vbox.add(pannableArea)
        ...
        response = dialog.run()
        if response == gtk.RESPONSE_OK:
            self.viewSearchResults(allExpense, allIncome, allSaving, searchphrase)
        dialog.destroy()

    def viewSearchResults(self, allExpense, allIncome, allSaving, searchphrase):
        ...
        def editDetail(menuItem, id, date, amount, category, tag, EorI):
            ...
            def getDate(self, widget):
                global dateNew
                dateNew = datetime.datetime.strptime(self.get_current_text(), '%A, %B %d, %Y').date()
            ...
            def selectionChangedCategory(self, widget):
                ...
                if tag is not None:
                    ...
                    cur.execute('select distinct Tag from Expenses where Category="'+categoryNew+'" order by Tag;')
                    c = cur.fetchall()
                    if len(c) > 0:
            ...
            def elements():
                ...
                buttonDate = hildon.DateButton(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
                ...
                buttonCategory = hildon.PickerButton(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
                ...
                if tag is not None:
                    ...
                    buttonTag = hildon.PickerButton(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
            ...
            pannableArea = hildon.PannableArea()
            pannableArea.add_with_viewport(elements())
            dialog = gtk.Dialog('Edit item', window, gtk.DIALOG_NO_SEPARATOR | gtk.DIALOG_DESTROY_WITH_PARENT)
            ...
            if response == gtk.RESPONSE_OK:
                if tag is not None:
                    cur.execute('update Expenses set Date="'+str(dateNew)+'", Expense="'+str(amountNew)+'", Category="'+categoryNew+'", Tag="'+tagNew+'" where Id="'+str(id)+'";')
                    db.commit()
                else:
                    if EorI == 'I':
                        cur.execute('update Incomes set Date="'+str(dateNew)+'", Income="'+str(amountNew)+'", Category="'+categoryNew+'" where Id="'+str(id)+'";')
                        db.commit()
                    elif EorI == 'S':
                        cur.execute('update Savings set Date="'+str(dateNew)+'", Saving="'+str(amountNew)+'", Category="'+categoryNew+'" where Id="'+str(id)+'";')
                        db.commit()
        ...
        def deleteDetail(menuItem, id, date, EorI):
            ...
            dialog = gtk.Dialog('', window, gtk.DIALOG_NO_SEPARATOR | gtk.DIALOG_DESTROY_WITH_PARENT)
            dialog.set_size_request(0, 200)
        ...
        def details(list, EorI):
            ...
            if EorI == 'E':
                for i in xrange(len(list)):
                    ...
                    menuItemEdit.connect('activate', editDetail, list[i][0], list[i][1], list[i][2], list[i][3], list[i][4], EorI)
                    menu.append(menuItemEdit)
                    menuItemDelete = gtk.MenuItem('Delete')
                    menuItemDelete.connect('activate', deleteDetail, list[i][0], list[i][1], EorI)
                ...
                for i in xrange(len(list)):
                    ...
                    menu.set_name('hildon-context-sensitive-menu')
                    menuItemEdit = gtk.MenuItem('Edit')
                    menuItemEdit.connect('activate', editDetail, list[i][0], list[i][1], list[i][2], list[i][3], None, EorI)
                    menu.append(menuItemEdit)
                    menuItemDelete = gtk.MenuItem('Delete')
                    menuItemDelete.connect('activate', deleteDetail, list[i][0], list[i][1], EorI)
                    menu.append(menuItemDelete)
        ...
        if len(allExpense) > 0 and len(allIncome) > 0 and len(allSaving) > 0:
        ...
        elif len(allExpense) > 0 and len(allIncome) == 0 and len(allSaving) == 0:
        ...
        elif len(allIncome) > 0 and len(allExpense) == 0 and len(allSaving) == 0:
            table = gtk.Table(9, 1, True)
            label = gtk.Label()
            ...
            table.attach(label, 0, 1, 0, 2)
            table.attach(pannableArea, 0, 1, 2, 9)
        elif len(allSaving) > 0 and len(allExpense) == 0 and len(allIncome) == 0:
            table = gtk.Table(9, 1, True)
            label = gtk.Label()
    ...
    def viewDay(self, widget, day):
        ...
        cur.execute('select sum(Expense) from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and strftime("%d", Date)="'+day+'";')
        list1.append(cur.fetchone()[0])
        cur.execute('select sum(Income) from Incomes where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and strftime("%d", Date)="'+day+'";')
        list2.append(cur.fetchone()[0])
        cur.execute('select sum(Saving) from Savings where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and strftime("%d", Date)="'+day+'";')
        list3.append(cur.fetchone()[0])
    ...
    def viewSum(self, list1, list2, list3, year, month, day):
        ...
        def onAct(treeview, path, data=None):
            ...
            if path[0] == 0:
                ...
                buttonByCategory = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_AUTO_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
                buttonByCategory.set_title('Expenses by category')
                buttonByCategory.connect('clicked', self.itemsByCategory, Expense, listE1, day)
                self.viewList(listE1, listE2, listE3, listDiffE, 'E', year, month, day)
            if path[0] == 1:
                ...
                buttonByCategory.connect('clicked', self.itemsByCategory, Income, listI1, day)
                self.viewList(listI1, listI2, listI3, listDiffI, 'I', year, month, day)
            if path[0] == 2:
                ...
                buttonByCategory.connect('clicked', self.itemsByCategory, Saving, listS1, day)
                self.viewList(listS1, listS2, listS3, listDiffS, 'S', year, month, day)

        treestore = gtk.TreeStore(gtk.gdk.Pixbuf, str, str, str, str, str)
        if list1[0] is not None:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/expenses.png'), 'Expenses ', '#f47500', str(list1[0])+' '+currency, '#f40005', '22'])
        else:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/expenses.png'), 'Expenses ', '#f47500', 'No expense', '#f40005', '22'])
        if list2[0] is not None:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/incomes.png'), 'Incomes ', '#f47500', str(list2[0])+' '+currency, '#00f475', '22'])
        else:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/incomes.png'), 'Incomes ', '#f47500', 'No income', '#00f475', '22'])
        if list3[0] is not None:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/savings.png'), 'Savings ', '#f47500', str(list3[0])+' '+currency, '#007ff4', '22'])
        else:
            treestore.append(None, [gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/savings.png'), 'Savings ', '#f47500', 'No saving', '#007ff4', '22'])
        treeview = gtk.TreeView(treestore)
        treeview.connect('row-activated', onAct)
        cellrenderer = gtk.CellRendererText()
        renderer = gtk.CellRendererPixbuf()
        treeview.append_column(gtk.TreeViewColumn('', renderer, pixbuf=0))
        treeview.append_column(gtk.TreeViewColumn('', cellrenderer, text=1, foreground=2, font=5))
        treeview.append_column(gtk.TreeViewColumn('', cellrenderer, text=3, foreground=4, font=5))
        return treeview
    ...
    def viewList(self, list1, list2, list3, listDiff, EorI, year, month, day):
        ...
        if listDiff is not None:
            treestore = gtk.TreeStore(str, str, str, str, str, str, gtk.gdk.Pixbuf)
        else:
            ...
            if listDiff is not None:
                if listDiff[i] == '':
                    ...
                    if listDiff[i] != '' and EorI == 'E':
                        ...
                        diffColor = '#00f475'
                if EorI == 'E':
                    iter = treestore.append(None, [list1[i], '#f47500', diff, diffColor, '16', '14', gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/goto.png')])
                else:
                    iter = treestore.append(None, [list1[i], '#f47500', diff, diffColor, '16', '14', None])
            else:
                #It is a bit ugly but until I add detail viewing function to Incomes this remains this way... and will give error message in terminal if user taps on the Pixbuf column of the treeview...
                if EorI == 'E':
                    iter = treestore.append(None, [list1[i], '#f47500', '16', gtk.gdk.pixbuf_new_from_file('/opt/maemoneypot/img/goto.png')])
                else:
                    iter = treestore.append(None, [list1[i], '#f47500', '16', None])
            if len(list3) != 0:
                for j in xrange(list3[i]):
                    if listDiff is not None:
                        treestore.append(iter, [list2[j+k], '#ffdcbd', None, '#ffffff', '14', None, None])
                    else:
                        treestore.append(iter, [list2[j+k], '#ffdcbd', '14', None])
                k = j+k+1
        treeview = gtk.TreeView(treestore)
        treeview.connect('row-activated', onAct)
        cellrenderer = gtk.CellRendererText()
        ...
        if listDiff is not None:
            treeviewcolumn = gtk.TreeViewColumn('', cellrenderer, text=0, foreground=1, font=4)
            treeviewcolumn1 = gtk.TreeViewColumn('', cellrenderer, text=2, foreground=3, font=5)
            ...
            treeviewcolumn = gtk.TreeViewColumn('', cellrenderer, text=0, foreground=1, font=2)
            treeview.append_column(treeviewcolumn)
    ...
    def viewDetails(self, category, sumExpense, day, path, treeview):
        ...
        def tagOrderQuery():
            if day is not None:
                cur.execute('select Id, Expense, Tag from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and strftime("%d", Date)="'+day+'" and Category="'+category+'" order by Tag;')
                window.set_title(category+' | '+day+' | '+MonthNameList[Month]+' | '+Year)
            else:
                cur.execute('select Id, strftime("%d", Date), Expense, Tag from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and Category="'+category+'" order by Tag;')
        ...
        def expenseOrderQuery():
            if day is not None:
                cur.execute('select Id, Expense, Tag from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and strftime("%d", Date)="'+day+'" and Category="'+category+'" order by Expense;')
                window.set_title(category+' | '+day+' | '+MonthNameList[Month]+' | '+Year)
            else:
                cur.execute('select Id, strftime("%d", Date), Expense, Tag from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and Category="'+category+'" order by Expense;')
                window.set_title(category+' | '+MonthNameList[Month]+' | '+Year)
            row = cur.fetchall()
            return row

        def dayOrderQuery():
            cur.execute('select Id, strftime("%d", Date), Expense, Tag from Expenses where strftime("%Y", Date)="'+Year+'" and strftime("%m", Date)="'+Month+'" and Category="'+category+'" order by strftime("%d", Date);')
            window.set_title(category+' | '+MonthNameList[Month]+' | '+Year)
            row = cur.fetchall()
            return row
        ...
        def editDetail(menuItem, id, d, expense, tag, order):
            ...
            response = dialog.run()
            if response == gtk.RESPONSE_OK:
                cur.execute('update Expenses set Date="'+str(dateNew)+'", Expense="'+str(expenseNew)+'", Category="'+categoryNew+'", Tag="'+tagNew+'" where Id="'+str(id)+'";')
                db.commit()
                if day is not None:
                    self.remove(TableCalendar)
                    self.calendarView(Year, Month)
        ...
        def deleteDetail(menuItem, id, expense, order):
            ...
            if response == 1:
                ...
                if sumDetailsExpense-expense == 0:
                    ...
                    label.set_text("<span size='24000'>"+str(sumDetailsExpense-expense)+" "+currency+"</span>")
                    if order == 'tag':
                        pannableArea.add_with_viewport(details(tagOrderQuery(), 'tag'))
                    elif order == 'expense':
                        pannableArea.add_with_viewport(details(expenseOrderQuery(), 'expense'))
                    elif order == 'day':
        ...
        def details(row, order):
            ...
            for i in xrange(len(row)):
                if day is not None:
                    ...
                    menu.set_name('hildon-context-sensitive-menu')
                    menuItemEdit = gtk.MenuItem('Edit')
                    menuItemEdit.connect('activate', editDetail, row[i][0], None, row[i][1], row[i][2], order)
                    menu.append(menuItemEdit)
                    menuItemDelete = gtk.MenuItem('Delete')
                    menuItemDelete.connect('activate', deleteDetail, row[i][0], row[i][1], order)
                    menu.append(menuItemDelete)
                    menu.show_all()
                    button.tap_and_hold_setup(menu)
                else:
                    ...
                    menu.set_name('hildon-context-sensitive-menu')
                    menuItemEdit = gtk.MenuItem('Edit')
                    menuItemEdit.connect('activate', editDetail, row[i][0], row[i][1], row[i][2], row[i][3], order)
                    menu.append(menuItemEdit)
                    menuItemDelete = gtk.MenuItem('Delete')
                    menuItemDelete.connect('activate', deleteDetail, row[i][0], row[i][2])
                    menu.append(menuItemDelete)
                    menu.show_all()
                    button.tap_and_hold_setup(menu)
        ...
        pannableArea = hildon.PannableArea()
        pannableArea.add_with_viewport(details(tagOrderQuery(), 'tag'))
        #buttonTagOrder = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL, 'Tag order')
        ...
        #buttonExpenseOrder = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL, 'Expense order')
        ...
        buttonTagOrder.set_active(True)
        graphDetails = matplotlibQuery.ExpensesByTags(sumExpense, Year, Month, day, category)
    ...
    def AddExpense(self, widget, day):
        ...
        def getDate(self, widget):
            global date
            date = datetime.datetime.strptime(self.get_current_text(), '%A, %B %d, %Y').date()
        ...
        def selectionChangedCategory(self, widget):
            ...
            selectorTag.remove_column(0)
            selectorTag.append_text_column(gtk.ListStore(str), 1)
            cur.execute('select distinct Tag from Expenses where Category="'+category+'" order by Tag;')
            c = cur.fetchall()
            if len(c) > 0:
                for i in xrange(len(c)):
                    selectorTag.insert_text(i, c[i][0])
        ...
        def save(data = None):
            if expense is None:
                note = hildon.Note('information', window, 'You have to specify expense!')
                gtk.Dialog.run(note)
                gtk.Dialog.hide(note)
            if category is None:
                note = hildon.Note('information', window, 'You have to specify category!')
                gtk.Dialog.run(note)
                gtk.Dialog.hide(note)
            if tag is None:
                note = hildon.Note('information', window, 'You have to specify tag!')
                gtk.Dialog.run(note)
                gtk.Dialog.hide(note)
            else:
                cur.execute('insert into Expenses(Date, Expense, Category, Tag) values("'+str(date)+'", "'+str(expense)+'", "'+str(category)+'", "'+str(tag)+'");')
                db.commit()
                iface.SystemNoteInfoprint('Expense was added to database')
                if category == settings[2][0:len(settings[2])-1]:
                    cur.execute('insert into Savings(Date, Saving, Category) values("'+str(date)+'", "'+str(expense)+'", "'+str(tag)+'");')
                    db.commit()
        ...
        buttonTag.set_relief(gtk.RELIEF_NONE)
        buttonTag.set_selector(selectorTag)
        buttonSave = hildon.Button(gtk.HILDON_SIZE_AUTO_WIDTH | gtk.HILDON_SIZE_FINGER_HEIGHT, hildon.BUTTON_ARRANGEMENT_VERTICAL)
        buttonSave.set_style(hildon.BUTTON_STYLE_NORMAL)
    ...
    def AddIncome(self, widget, day):
        ...
        def save(data = None):
            if income is None:
                note = hildon.Note('information', window, 'You have to specify income!')
                gtk.Dialog.run(note)
                gtk.Dialog.hide(note)
            if category is None:
                note = hildon.Note('information', window, 'You have to specify category!')
                ...
                cur.execute('insert into Incomes(Date, Income, Category) values("'+str(date)+'", "'+str(income)+'", "'+str(category)+'");')
                db.commit()
                bus = dbus.SystemBus()
                iface = dbus.Interface(bus.get_object('org.freedesktop.Notifications', '/org/freedesktop/Notifications'), 'org.freedesktop.Notifications')
    ...
    def AddSaving(self, widget, day):
        ...
        def save(data = None):
            ...
            if category is None:
                ...
                cur.execute('insert into Savings(Date, Saving, Category) values("'+str(date)+'", "'+str(saving)+'", "'+str(category)+'");')
                db.commit()
                cur.execute('insert into Expenses(Date, Expense, Category, Tag) values("'+str(date)+'", "'+str(saving)+'", "Saving", "'+str(category)+'");')
                db.commit()
                bus = dbus.SystemBus()
                iface = dbus.Interface(bus.get_object('org.freedesktop.Notifications', '/org/freedesktop/Notifications'), 'org.freedesktop.Notifications')
                iface.SystemNoteInfoprint('Income was added to database')
...
if __name__ == '__main__':
    if len(settings[0]) == 1 and len(settings[1]) == 1 and len(settings[2]) == 1:
        ...
        if defaultView == 'Main':
            MainWindow().mainView(time.strftime('%Y'), time.strftime('%m'), None)
        elif defaultView == 'Calendar':
